...
class News:
    ...
    def __getDescriptions__(self, url_feed): #функция для получения описания новости
        descriptions = []
        lenta = self.__check_url__(url_feed)
        for item_of_news in lenta['items']:
    ...
    def write_all_news(self, all_news_filepath): #функция для записи всех новостей в .csv, 
    # возвращает нам этот датасет
        ...
        with open(all_news_filepath, 'w', encoding='utf-8-sig') as csvfile:
            ...
            for a,b,c,d  in zip(self.allheadlines,self.alldescriptions,
                                self.alllinks, self.alldates):
    ...
    def looking_for_certain_news(self, all_news_filepath, certain_news_filepath, target1, target2): #функция для поиска, а затем записи
